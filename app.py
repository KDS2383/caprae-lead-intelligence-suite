...
# New imports for Signal Monitoring
...
def check_single_company_for_signal(driver, company_name, job_title):
    """Uses Selenium to check for a job posting signal."""
    # The text Google displays when no job results are found.
    NO_RESULTS_TEXT = "did not match any job postings"
    
    # We use the company name from the URL for better search results
    clean_company_name = re.sub(r'https?://(www\.)?', '', company_name).split('/')[0].split('.')[0]

    query = f'site:linkedin.com/jobs "{job_title}" "{clean_company_name}"'
    search_url = f"https://www.google.com/search?q={query.replace(' ', '+')}"
    
    try:
        driver.get(search_url)
        # A short sleep to be polite and allow content to load
        # time.sleep(1) 
        page_content = driver.page_source.lower()
        
        if NO_RESULTS_TEXT.lower() in page_content:
            return "No"
        else:
            return "Yes"
    except Exception as e:
        print(f"Error checking signal for {company_name}: {e}")
        return "Error"
...
def check_signals():
    ...
    # URLs are passed as a single string, separated by newlines
    urls = [url.strip() for url in data.get('urls', '').split('\n') if url.strip()]

    if not job_title or not urls:
        return jsonify({"error": "Job title and company URLs are required."}), 400

    driver = setup_selenium_driver()
    if not driver:
        return jsonify({"error": "Server could not initialize signal checker."}), 500

    signal_results = []
    # Running concurrently to speed up the process
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_url = {executor.submit(check_single_company_for_signal, driver, url, job_title): url for url in urls}
        for future in concurrent.futures.as_completed(future_to_url):
            url = future_to_url[future]
            try:
                signal_detected = future.result()
                ...
                signal_results.append({"url": url, "signal_detected": "Error"})
    
    driver.quit()
    return jsonify(signal_results)
